...
dnbvar = [zeros(ndnb(1),1) zeros(ndnb(2),1)];

for i = 1:size(dnbdata_1,2)
     dnbvar(1,i) = std(dnbdata_1(:,i));
end

for i = 1:size(dnbdata_2,2)
     dnbvar(2,i) = std(dnbdata_2(:,i));
end

goodrange = 1:4064;
size(goodrange)
size(dnbvar(1,goodrange))

p_1 = polyfit(double(goodrange),dnbvar(1,goodrange)',3)
disp(dnbvar(1));
polyvar_1 = polyval(p_1,goodrange);

p_2 = polyfit(double(goodrange),dnbvar(2,goodrange)',3)
