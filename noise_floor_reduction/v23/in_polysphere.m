...
%            matrix) is inside (1) or on/outside the polygon (0).
% Tested : Matlab 7.11
...
FlagMat = zeros(size(Positions,1),Nvert-1);
for Ivert=1:1:Nvert,
    % cross (X) two verteces in order to get the pole of the great
    % circle defined by the two verteces
    PoleVec = cross(Corners(Ivert,:),Corners(Ivert+1,:),2);
    % normalize polar vector to unity
    PoleVec = PoleVec./sqrt(sum(PoleVec.^2));

    FlagMat(:,Ivert) = in_halfspace(Positions,PoleVec,0,1,Crit);
