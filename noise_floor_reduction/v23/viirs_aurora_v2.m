...
sigmanoise = zeros(size(imageDNB));
for j=1:size(imageDNB,1)
    sigmanoise(j,1:4064) = polyval(PNOISE,1:4064)';
end

imageDNB = wiener2(imageDNB,[3 3],64*sigmanoise);

%% Aurora flux model file
try
    model = viirs_aggregate_granules_simple(aimap, '')';
catch
    error('Missing Aurora Model file')
end

%%
% for log(DNB)
% auind = find(model > 0.1 & img > model * 0.05 + 0.75);
% for linear DNB (no log)
% auind = find(model > 0.1 & img > 0.3 * model.^2 + 1.4);
auind = find(model > 0.5 & imageDNB > model + 0.9);
auseeds = zeros(size(imageDNB));
auseeds(auind) = 1;

% remove isolated features less than P pixels
P = 100;
auseeds = bwareaopen(auseeds,P);

% Get all the blob properties.  Can only pass in originalImage in version R2008a and later.
labeledImage = bwlabel(auseeds, 8);
blobMeasurements = regionprops(labeledImage, imageDNB, 'all');
...
% figure
% plot(allBlobAreas,allBlobIntensities,'.')
% xlabel('Area')
% ylabel('DNB max')

% Get a list of the blobs that meet our criteria and we need to keep.
allowableIntensityIndexes = (allBlobIntensities > 5); % 
allowableAreaIndexes = allBlobIntensities > allBlobAreas / 100; % Take the small objects.
...
dropIndexes = find((allowableIntensityIndexes & allowableAreaIndexes));
ndrop = length(dropIndexes)
...
dropBlobsImage = ismember(labeledImage, dropIndexes);

auseeds = keeperBlobsImage;

% find valid pixels
sunlitflag = ones(size(imageDNB));
sunlitflag(sunlit) = 0;
% sunlitflag(:,1:226) = 0;
% sunlitflag(:,3841:end) = 0;

% reconsrtuct DNB image marked with aurora streaks
imgAurora = imreconstruct(double(auseeds).*stretchDNB,stretchDNB);

% stretchDNB(:,1:226) = 0;
% stretchDNB(:,3841:end) = 0;

imgtmp = imgAurora;
imgAurora(sunlitflag == 0) = 0;

subimgAurora = imgAurora;
subimgAurora(sunlitflag > 0) = 0;
% subimgAurora = subimgAurora(:,257:3840);
size(subimgAurora)

% imwrite(imgAurora,'aurora.png')

thrAurora = thrOtsu(imgAurora(sunlitflag > 0),256)
% thrAurora = thrTriangle(imgAurora(sunlitflag > 0),256)
if thrAurora > 0.1
    if thrAurora > 0.6;
        imgAurora = im2bw(imgAurora,thrAurora);
    else
        imgAurora = im2bw(imgAurora,0.6);        
    end
else 
    imgAurora = zeros(size(imgAurora));
end
...
if visFlag
    ...
    figaurora = figure('Position',[1 1 scrsz(3) scrsz(4)],'Name','Glint specle detector');
    title(outfile)
    ...
    dtctrgb = zeros(size(imageDNB,1),size(imageDNB,2),3);
    dtctrgb(:,:,1) = min(imgtmp*0.9,ones(size(stretchDNB)));
    dtctrgb(:,:,2) = min(imgtmp*0.9,ones(size(stretchDNB))).*~imgAurora;
    dtctrgb(:,:,3) = min(imgtmp*0.9,ones(size(stretchDNB)));

    subplot(2,2,4)
    image(dtctrgb)
    axis image
    colormap('gray');
    title(['Aurora and DNB image overlay thr=' num2str(thrAurora)]);
    freezeColors;
    ...
    print(figaurora,'-dpng','-r0',[outfile,'-aurora.png']);    
end
