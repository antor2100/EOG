...
disp('Flare mask read')
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(5).mem = usedMemory;
tmemprof(5).time = usedTime;
tmemprof(5).step = 'flares';
...
disp('Lightning detector')
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(6).mem = usedMemory;
tmemprof(6).time = usedTime;
tmemprof(6).step = 'lightning';
...
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(7).mem = usedMemory;
tmemprof(7).time = usedTime;
tmemprof(7).step = 'streaks';
...
if dumpFlag && dodump
    info=enviinfo(uint8(glintim + glintspecl));
    enviwrite(uint8(glintim + glintspecl),info,[outfile '.glint_specular']);
end

% if dumpFlag
%     info=enviinfo(glintDNB);
%     enviwrite(glintDNB,info,[outfile '.glint_specle']);

%     info=enviinfo(stepDNB);
%     enviwrite(stepDNB,info,[outfile '.glint_step']);
    %     info=enviinfo(prodDNB1);
    %     enviwrite(prodDNB1,info,[outfile '.lightning_prod1']);
    %     info=enviinfo(prodDNB2);
    %     enviwrite(prodDNB2,info,[outfile '.lightning_prod2']);
...
disp('DNB/I05 correlator')    
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(9).mem = usedMemory;
tmemprof(9).time = usedTime;
tmemprof(9).step = 'xcorr';
...
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(10).mem = usedMemory;
tmemprof(10).time = usedTime;
tmemprof(10).step = 'blur';
...
% 
%     subplot(2,2,3)
%     plot(glintdata(glint_map).*limage(glint_map),imageDNB(glint_map),'.')
%     hold on
%     title('Glint /DNB scatter');
% 
% end
% 
% return
...
%     size(imgfilt(slval > 0))
    ...
    for i = 1:numel(edges)-1
        ...
        hold on
        plot(smili(noiseind),xcorli(noiseind),'r.')
...
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(11).mem = usedMemory;
tmemprof(11).time = usedTime;
tmemprof(11).step = 'shi';
...
for i = 1:ilimit
    ...
    if streakDNB(r(i),c(i)) == 0 && ...
        ...
        if isOcean(i) == 1
            ...
            if spike_map(r(i),c(i)) >= 0.995 && dnb > 1000
            ...
            if ~isnan(flareval) && flareval
                ...
                glow = bitand(uint32(flareval),uint32(4)); % bitflag 4 in the flaremap raster
            ...
            if true
                ...
                elseif spike_map(r(i),c(i)) >= 0.995 && dnb > 1000
                    if visFlag
                        plot(c(i),r(i),'mo')
                    end
                    coords(i,4) = 5; % cosmic ray                
                elseif glint_map(r(i),c(i)) && (imgfilt(r(i),c(i)) < glinttop  || ...
                       glintDNBthr ~= FILLVALUE && imageDNB(r(i),c(i)) < glintDNBthr)
                    if visFlag
                        plot(c(i),r(i),'co')
                    end
...
if false
    figure
    imagesc(lizones+slightimg)
    hold on
    plot(coords(isBoat == 1,9),coords(isBoat == 1,8),'r+')
    axis image
    colormap('gray');
    colorbar;
    title('Boat detections over LI zones')
end
...
disp(['Found ', num2str(count), ' boats']);
usedTime = toc
usedMemory = rtime.totalMemory - rtime.freeMemory
tmemprof(12).mem = usedMemory;
tmemprof(12).time = usedTime;
tmemprof(12).step = 'loop';
...
if count > 0 && ~isempty(pfm)
    ...
    % Some example coordinates (degrees are assumed)
        ...
        % Your point of interest lon,lat
        ...
        if minDist < PLP_RADIUS
            ...
            if ismember(coords(bind(i),4), [1,2,3]);
                coords(bind(i),4) = 8; % QF8 for recurring source
            end
        end
    end
    ...
    disp('recurring lights search done')
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(13).mem = usedMemory;
    tmemprof(13).time = usedTime;
    tmemprof(13).step = 'rlp';
end
...
if count > 0 && ~isempty(brg)
    ...
    brind123 = ismember(coords(bind,4), [1,2,3]) & ~isnan(bridgeid);
    coords(bind(brind123),4) = 8; % QF8 for recurring source
    
    disp(['Bridge vectors search done, found ', num2str(length(brind)), ' lights'])
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(14).mem = usedMemory;
    tmemprof(14).time = usedTime;
    tmemprof(14).step = 'rlv';
...
if count > 0 && ~isempty(eez)
    eeznames(isBoat == 1) = interpshapefile(eez, coords(isBoat == 1,1), coords(isBoat == 1,2), 'EEZ', 'Y', 'X');
    
    disp('EZZ name search done')
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(15).mem = usedMemory;
    tmemprof(15).time = usedTime;
    tmemprof(15).step = 'eez';
end

if count > 0 && ~isempty(fmz)
    fmznames(isBoat == 1) = interpshapefile(fmz, coords(isBoat == 1,1), coords(isBoat == 1,2), 'CATEGORY', 'Y', 'X');
    
    disp('FMZ name search done')
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(16).mem = usedMemory;
    tmemprof(16).time = usedTime;
    tmemprof(16).step = 'fmz';
end

if count > 0 && ~isempty(mpa)
    mpanames(isBoat == 1) = interpshapefile(mpa, coords(isBoat == 1,1), coords(isBoat == 1,2), 'CATEGORY', 'Y', 'X');
    
    disp('MPA name search done')
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(17).mem = usedMemory;
    tmemprof(17).time = usedTime;
    tmemprof(17).step = 'mpa';
end
...
if ~isempty(dtctind) && ~isempty(ltz)
    ...
    disp('Local time zones search done')
    usedTime = toc
    usedMemory = rtime.totalMemory - rtime.freeMemory
    tmemprof(18).mem = usedMemory;
    tmemprof(18).time = usedTime;
    tmemprof(18).step = 'ltz';
end
...
%     end
    ...
    fprintf(csvfile,'id,id_Key,Date_Proc,Lat_DNB,Lon_DNB,Rad_DNB,Date_Mscan,Date_LTZ,Line_DNB,Sample_DNB,QF_Detect,QF_Bitflag,');
    fprintf(csvfile,'SMI,Thr_SMI,SI,Thr_SI,SHI,Thr_SHI,LI,Thr_LI,Glint,Thr_Gl_SMI,Thr_Gl_DNB,Xcorr,');
    fprintf(csvfile,'Land_Mask,EEZ,FMZ,MPA,File_DNB,File_GDNB,File_VNF,File_EEZ,File_FMZ,File_MPA,File_FLM,File_LSM,');
...
%             fprintf(csvfile,'%d,%s,%s,%f,%f,%g,%s,%s,%d,%d,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%d,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%g,%s,%s,%s,%s,%g,%g,%g,%g,%g,%g\n',....
            fprintf(csvfile,'%d,%s,%s,%f,%f,%g,%s,%s,%d,%d,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%d,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%g,%s,%s,%s,%s,%g,%g,%g,%g,%g,%g\n',....
                id,id_key,procdate,coords(i,1),coords(i,2),coords(i,3),dtstr,localdtstr, ...
                coords(i,8)-granedges(nf),coords(i,9),coords(i,4),coords(i,13),coords(i,11),coords(i,12),coords(i,5),SI,coords(i,6),SHI,coords(i,14),MOONLIT,coords(i,16),coords(i,17),glintDNBthr,coords(i,15),coords(i,10), ...
                waters,fishery,protected,[nameDNB,extDNB],[nameGeo,extGeo],[nameCSV,extCSV],[nameEEZ,extEEZ],[nameFMZ,extFMZ],[nameMPA,extMPA],...
    ...
    if id == 0 % no detections at all
        fprintf(csvfile,'%d,%s,%s,%f,%f,%g,%s,%s,%d,%d,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%d,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%g,%s,%s,%s,%s,%g,%g,%g,%g,%g,%g\n',....
            id,'',procdate,FILLVALUE,FILLVALUE,FILLVALUE,'','',FILLVALUE,FILLVALUE,...
            FILLVALUE,FILLVALUE,FILLVALUE,SMI,FILLVALUE,SI,FILLVALUE,SHI,FILLVALUE,MOONLIT,FILLVALUE,FILLVALUE,FILLVALUE,FILLVALUE,...
            FILLVALUE,'','','',[nameDNB,extDNB],[nameGeo,extGeo],[nameCSV,extCSV],[nameEEZ,extEEZ],[nameFMZ,extFMZ],[nameMPA,extMPA],...
...
%     end
    ...
    write_detection_kml_folders_v22g(idvals,idkeys,coords(granind,:),eeznames(granind),fmznames(granind),mpanames(granind),...
