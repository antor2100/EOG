function outfile = heatscatter(X, Y, slope, outpath, outname, numbins, markersize, marker, plot_colorbar, plot_lsf, xlab, ylab, title)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~exist('outname','var') || isempty(outname)
        warning('No outname provided, the figure will not be saved');
        saveFig = false;
%         error('Param outname is mandatory! --> EXIT!');
    ...
    f = figure('Name','Heatscatter');
    scatter(X, Y, markersize, scatter_COL, marker);
    hold on
    plot([min(X),max(X)],slope*[min(X),max(X)],'k');
    [min(X),max(X)]
    slope*[min(X),max(X)]
