function boats_reduce_v20g(FILELIST,varargin)
%
...
% FILELIST = 'paffy_clouds copy.txt'
% OUTPUT = 'paffy_clouds.csv'
% MAP = 1
% COVERAGE = 0
% STRICT = 0
% SORT = 0
...
%         size(localdate)
    ...
    write_detection_kml_folders_v20g(id,cellstr(id_key),coords,eeznames,fmznames,mpanames,latgring,longring,doCoverage,~strictFlag,FILLVALUE,kmlfile,flist,boats_version);    
end

header = 'id,id_Key,Date_Proc,Lat_DNB,Lon_DNB,Rad_DNB,Date_Mscan,Date_LTZ,Line_DNB,Sample_DNB,';
header = [header,'SMI,Thr_SMI,SI,Thr_SI,SHI,Thr_SHI,LI,Thr_LI,Glint,Thr_Glint,Xcorr,QF_Detect,QF_Bitflag,'];
header = [header,'Land_Mask,EEZ,FMZ,MPA,File_DNB,File_GDNB,File_VNF,File_EEZ,File_FMZ,File_MPA,File_FLM,File_LSM,'];
header = [header,'File_LTZ,Lat_Gring,Lon_Gring,Gran_List\n'];

format = '%d,%s,%s,%f,%f,%g,%s,%s,%d,%d,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%d,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n';
