function value = interpshapefile(S, lat, lon, attribute, latname, lonname)
%INTERPSHAPEFILE Interpolates data based on a shapefile data structure
%
% value = interpshapefile(S, lat, lon, attribute, latname, lonname)
%
...
%
%   latname:        name of attribute field with latitude (Y) coordinates
%
%   lonname:        name of attribute field with longitude (X) coordinates
%
% Output variables:
%
...
% Modified for arbitrary 'latname' and 'lonname' 2014 Mikhail Zhizhin
% Changed how the single polygon from a (possibly multiple) list for each
% point is selected 2018 Mikhail Zhizhin

%------------------------------
% Check input
%------------------------------

if ~isstruct(S) || ~all(isfield(S, {latname, lonname, attribute}))
...
if ~all(size(lat) == size(lon))
    error('lat and lon must be matrices of the same size');
end
...
nclockwise = cellfun(@(x,y) length(find(ispolycw(x,y))), {S.(lonname)}, {S.(latname)});
repVals = cellfun(@(nrep, val) repmat({val},1,nrep), num2cell(nclockwise), {S.(attribute)}, 'UniformOutput', false);
repVals = [repVals{:}];
 
[in, indexCell] = inpolygons(lon, lat, [S.(lonname)], [S.(latname)]);

% try
%     index = cell2mat(indexCell);
% catch
%     warning('Overlapping polygons; using first match for each point');
