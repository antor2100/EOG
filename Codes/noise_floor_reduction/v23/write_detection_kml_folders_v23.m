function write_detection_kml_folders_v23(id,idkey,coords, eeznames, fmznames, mpanames, latgring, longring, ...
...
fprintf(fid,'<Folder>\n');
fprintf(fid,'<name>Platforms</name>\n');
fprintf(fid,'<open>0</open>\n');
filesav = '';
for i=1:size(coords,1)
    ...
    if coords(i,4) == 11
        ...
        write_kml_description(fid,i,verbose,'Platform',dtstr,localdtstr,flarename,coords,waters,fishery,protected,landsea,dnbname);       
...
if docoverage && length(id) > 0
    ...
    for i=1:size(latgring,1)
        if ~ (strcmp(latgring(i,:),latsav) && strcmp(longring(i,:),lonsav))
            ...
            strfname = strtrim(dnblist(i,:));
            obsdate = dateFromFileName(strfname);
            
            if docoverage == 1
%                 covimage_new = viirs_coverage(latvec,lonvec,10);
                covimage_new = viirs_coverage(latvec,lonvec,10,obsdate,105);
