...
function [] = sat_9_concat()
    ...
    for i = 1:n
        name = input(['enter filename ' num2str(i) ': ']);
        ...
        if i == 1
            dnbdata = viirs_get_data(name, dnbdataset)' * 1e9;
            dnbdata(:,1) = [];
        
        else

            read_dnbdata = viirs_get_data(name, dnbdataset)' * 1e9;
    ...
    while k <= size(dnbdata_3,2)
        j = 1;
        while j <= size(dnbdata_3,1)
            if dnbdata_3(j,k) > m + 1
                dnbdata_3(:,k) = [];
                goodrange_2(k) = [];
                flag = 0;
                break;
    ...
    %% Create another array to turn the spikes into dots
    hold on
    ...
    for j = 1:size(dnbdata_3,2)
         dnbvar_3(j) = std(dnbdata_3(:,j));
    end
    ...
    goodrange_3 = 1:size(dnbdata_3,2);
    ...
    p_2 = polyfit(double(goodrange),dnbvar_2(goodrange)',4);

    sigmanoiseSNPP_2 = polyval(p_2,1:4063);

    % for fitting polynomial

    p_3 = polyfit(double(goodrange_2),dnbvar_3(goodrange_3)',degree);

    polyvar_3 = polyval(p_3,goodrange);

    sigmanoiseSNPP_3 = polyval(p_3,1:4063);

    p_3  

    %%
    SCALE = 1;
    af3 = single(wiener2(dnbdata,[3 3],SCALE*sigmanoiseSNPP_3));
    ...
    % with spikes
    imgmed = medfilt2(af3_2,[3 3]);
    imgfiltSNPP = (af3_2 - imgmed);
