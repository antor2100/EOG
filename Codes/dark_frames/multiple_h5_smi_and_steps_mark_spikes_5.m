%%
function [] = multiple_h5_smi_and_steps_mark_spikes_5()
    SMI = 0.095*1.75;
    ...
    for i = 1:n
        ...
        dnbdata_3(:,(b+2-a):size(dnbdata_3,2)) = [];
        ...
        while j <= size(dnbdata_3,2)
            k = 1;
            while k <= size(dnbdata_3,1)
                if dnbdata_3(k,j) > m + 1
                    dnbdata_3(:,j) = [];
                    goodrange_2(j) = [];
                    flag = 0;
                    break;
        ...
        sigmanoiseSNPP_2 = polyval(p_2,1:4064);
