%%
function [] = multiple_h5_smi_and_steps_mark_spikes_concat(sat_type, SMI, degree)
	...
	F = getframe(gcf);
	im = frame2im(F);
	imwrite(im, 'images/concat_smi.png');
    ...
    F = getframe(gcf);
	im = frame2im(F);
	imwrite(im, 'images/concat_bowtie.png');
    ...
    plot(goodrange,dnbvar(goodrange),'b')
    legends{end+1} = 'concatenated';
    hold on
    plot(goodrange,polyvar(goodrange),'g')
    ...
    F = getframe(gcf);
	im = frame2im(F);
	imwrite(im, 'images/concat_std.png');
