%%
function [p_1] = sat_3(type, name, degree)
    ...
    opts = detectImportOptions('polynomials_2.csv','NumHeaderLines',0);
    output = readtable('polynomials_2.csv', opts);
    disp(output)
    ...
    name = split(name,"_");
    YMD = extractAfter(name(4),1);
    YMD = str2double(YMD);
    
    id = strcat(name(7), "_", name(8));
    
    row = {type, YMD, name(5), name(6), id, degree, coeff, 0};
    output = [output; row];
