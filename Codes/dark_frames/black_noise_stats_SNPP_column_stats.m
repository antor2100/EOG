...
dsmimin = zeros(ndnb,1);
dsmimean = zeros(ndnb,1);
dsmistd = zeros(ndnb,1);

for i = 1:size(dnbdata,2)
    dsmimax(i) = max(imgfiltSNPP(:,i));
    dsmimin(i) = min(imgfiltSNPP(:,i));
    dsmimean(i) = mean(imgfiltSNPP(:,i));
    dsmistd(i) = std(imgfiltSNPP(:,i));
end
...
hold on
plot(goodrange,dsmimin(goodrange))
hold on
plot(goodrange,dsmimean(goodrange))
hold on
plot(goodrange,dsmistd(goodrange))
hold on
plot(goodrange,polyvar, 'r')
% plot(goodrange,polyvar+delta,'r.')
xlim([-50,ndnb+50])
